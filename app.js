...
function renderBadgePriority(p) {
  ...
  span.title = 'Клікніть, щоб змінити пріоритет';
  return span;
}
...
function makeItemNode(item) {
  ...
  btnMore.addEventListener('click', (e) => {
    ...
    if (expanded) {
      ...
      btnMore.textContent = 'Показати менше';
  ...
  init() {
    ...
    if (COLUMNS.every(c => state[c].length === 0)) {
      places[0].append(
        makeItemNode({
          id: uid(),
          title: 'Спробуй перетягнути мене',
          priority: 'med',
          createdAt: Date.now()
        }),
        makeItemNode({
    ...
    if (toggle) {
      ...
      toggle.textContent = 'Автосортування: ' + (on ? 'увімк' : 'вимк');
      toggle.addEventListener('click', () => {
        const now = !isAutoSort();
        setAutoSort(now);
        toggle.setAttribute('aria-pressed', now ? 'true' : 'false');
        toggle.textContent = 'Автосортування: ' + (now ? 'увімк' : 'вимк');
        if (now) sortAll();
        Board.syncFromDOMandSave();
      });
    }
